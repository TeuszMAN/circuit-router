...
describe('LevelSelect', () => {
  it('agrupa as 24 fases nos seis capítulos reais e mantém progresso e seleção', () => {
    const state = createAppState()
    state.recordResult('p1-1', {
      stars: 2,
      pieces: 1,
      gates: 0,
      withHint: false,
    ...
    expect(within(chapters[0]!).getAllByRole('button')).toHaveLength(3)
    expect(screen.getAllByRole('button')).toHaveLength(25)
    expect(screen.getByRole('progressbar').getAttribute('value')).toBe('1')
    fireEvent.click(screen.getByRole('button', { name: /^Somador completo/ }))
    expect(onPick).toHaveBeenCalledWith(campaign.summaries.at(-1)!.id)
  })
  it('lista as fases recebidas por array fake', () => {
    ...
    for (const level of FAKE_LEVELS) {
      expect(
        screen.getByRole('button', { name: new RegExp(level.name) }),
      ).toBeTruthy()
    }
  ...
  it('mostra as estrelas do progresso de cada fase', () => {
    ...
    expect(
      within(first).getByRole('img', {
        name: 'Estrelas de Primeira rota: 2 de 3 estrelas',
      }),
    ).toBeTruthy()
    ...
    expect(
      within(second).getByRole('img', {
        name: 'Estrelas de Negações: 0 de 3 estrelas',
      }),
    ).toBeTruthy()
  })
  ...
  it('preserva o progresso (estrelas + selo) entre renders do estado', () => {
    ...
    expect(
      within(third).getByRole('img', {
        name: 'Estrelas de Só os dois: 3 de 3 estrelas',
      }),
    ).toBeTruthy()
