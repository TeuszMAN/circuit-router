...
7. **Naming:** **"Circuit Router"** em todos os lugares — `<title>`, `<h1>`, manifest PWA, README, SDD, ADRs. `package.json` raiz já é `circuit-router`; workspaces sob o escopo `@circuit/*`. Toda ocorrência de "Logic Router" é erro a corrigir.
